...
// Layout constants — keep these in sync with your TopNavbar / DashSideBar heights
const TOP_NAV_HEIGHT = 120;
const DESKTOP_NAV_HEIGHT = 20;
const MOBILE_BOTTOM_NAV_HEIGHT = 10;

const StudentMessages = () => {
    // Desktop keeps chat 1 pre-selected; mobile starts with null (list view)
...
const ChatListItem = ({ chat, isActive, onSelect, mobile }: any) => (
    <button
        onClick={onSelect}
        className={`w-full box-border ${mobile ? "px-4" : "px-8"} py-4 border-b border-gray-50 text-left hover:bg-gray-50 transition-all ${isActive ? "bg-emerald-50 border-l-4 border-l-emerald-600" : ""
            }`}
    >
        <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-emerald-600 font-medium mb-1 truncate">{chat.subject}</p>
                <p className={`text-xs truncate ${chat.unread ? "text-gray-900 font-medium" : "text-gray-600"}`}>
                    {chat.lastMessage}
                </p>
            </div>

            {chat.unread && <div className="w-2.5 h-2.5 bg-emerald-600 rounded-full flex-shrink-0 mt-1"></div>}
        </div>
    </button>
);
...
const ChatWindow = ({
    ...
    return (
        <div className="flex flex-col bg-gray-50 h-full overflow-hidden">
            {/* Header */}
            <div className="mt-14 px-4 sm:px-8 py-4 border-b bg-gray-100 border-gray-200 flex items-center justify-between  flex-shrink-0 z-20">
                <div className="flex items-center gap-3 min-w-0">
                    ...
                    >
                        ...
                        <div className="min-w-0">
                            <h2 className="font-bold text-sm text-gray-900 text-lg truncate">{currentChat?.name}</h2>
            ...
            {/* Input bar */}
            <div className="px-4 sm:px-8 py-3 border-t border-gray-100 bg-white flex-shrink-0 z-20 bottom-0">
